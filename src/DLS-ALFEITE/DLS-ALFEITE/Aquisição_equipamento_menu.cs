...
namespace DLS_ALFEITE
{
    public partial class Aquisição_equipamento_menu : Form
    {
        ...
        bool check = false;
        public Aquisição_equipamento_menu(bool value)
        {
            InitializeComponent();
            Form_estilo();
            check = value;
        ...
        private void btn_aprovar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                using (cmd = new SqlCommand("DELETE FROM Aquisicao_equipamentos WHERE id_aquisicao = @id", sqlCon))
                {
                    cmd.Parameters.AddWithValue("@id", pictureBox1.Tag);
                    sqlCon.Open();
                    cmd.ExecuteNonQuery();
